    ...
    <>
      <div className="mt-10 ">
        ...
        {showMenu && (
          <div>
            <nav className="mt-4">
              <ul className="fixed py-3 z-11 inset-0 w-[85%] gap-4 mt-10 bg-bg-menu mx-auto rounded-2xl">
                <li
                  onClick={() => {
                    setShowMenu(false);
                  }}
                ...
                {links?.map((e, i) => (
                  <li
                    key={i}
                    className="capitalize md:text-[20px] text-[14px] mx-auto hover:text-main-light hover:text-[20px]"
                  >
                    <a href={e?.href}>{e?.name}</a>
                  </li>
                ))}
