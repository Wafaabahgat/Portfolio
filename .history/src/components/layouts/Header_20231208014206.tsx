    ...
    <>
      <div className="mt-10 ">
        ...
        {showMenu && (
          <div>
            <nav className="mt-4">
              <ul className="fixed py-3 z-11 inset-0 w-[85%] mt-10 bg-bg-menu mx-auto rounded-2xl">
                <li
                  onClick={() => {
                    setShowMenu(false);
                  }}
                  className="bg-white text-bg-header rounded-full w-[20px] h-[20px] flex items-center justify-center gap-10"
                >
                  <IoMdClose />
                </li>
                {links?.map((e, i) => (
                  <li
                    key={i}
                    className="capitalize text-[20px] mx-auto hover:text-main-light hover:text-[25px]"
                  >
                    <a href={e?.href}>{e?.name}</a>
                  </li>
                ))}
